...
class CrAFT(nn.Module):
    def __init__(
        ...
        *,
        ...
        mlpfoly_hidden = ff_hidden
        print(f'mlp_hidden: {mlpfoly_hidden}')
        self.mlpfory = simple_MLP([dim ,mlpfoly_hidden, y_dim])
        self.pt_mlp = simple_MLP([dim*(self.num_continuous+self.num_categories+((cross_nhead)*depth)) ,6*dim*(self.num_continuous+self.num_categories+((cross_nhead)*depth))//5, dim*(self.num_continuous+self.num_categories)//2])
        self.pt_mlp2 = simple_MLP([dim*(self.num_continuous+self.num_categories+((cross_nhead)*depth)) ,6*dim*(self.num_continuous+self.num_categories+((cross_nhead)*depth))//5, dim*(self.num_continuous+self.num_categories)//2])# 要修正: 1 >> cross_nhid*cross_nhead
